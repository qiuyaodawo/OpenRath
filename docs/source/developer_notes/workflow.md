...
| `src/rath/flow/compressor.py` | `Compressor` preset workflow。 |
...
| `Compressor` | 对已有 session 调用一次 `run_session_compress`。 |
...
workflow(session)
  ...
  -> optional Agent.forward / Compressor.forward
